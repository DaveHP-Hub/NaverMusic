import { Component, ViewChild, ElementRef} from '@angular/core';
import { HomeService } from '../../services/home.service';
import { Router } from "@angular/router";
import { FormGroup, FormBuilder} from '@angular/forms'; 
import {DatePipe} from '@angular/common';



@Component({
  selector: 'app-home',
  templateUrl: './home.component.html',
  styles: [
  ]
})
export class HomeComponent {

$: any;

frmFavorite: FormGroup;

newReleases: any[] = [];
chartsDeezer: any[] = [];
typefav: string;
idfav_: string;
iduser_: number;
namefav_: string;
picture_: string;
namesong_:string;
nameapi: string;
uri: string;
date = new Date();


latestpipe;

id: number;
dato = localStorage.getItem('id');

  constructor(private serviceHome: HomeService, private router: Router, private _fb: FormBuilder, public datepipe: DatePipe) 
  {

 this.serviceHome.getNewReleases()
        .subscribe((data: any) => {

        this.newReleases = data;
       
        })

        
   this.serviceHome.getChartsDeezer()
    .subscribe((data: any) => {
    this.chartsDeezer = data.tracks.data;
    })

       
  }


 


  seeArtist(item: any) {
let artistId;
if(item.track.artists.type === 'artist')
{
artistId = item.track.artists.id;

}
else{
  artistId = item.track.artists[0].id;
 
}

this.router.navigate(['/spotify-artist', artistId]);


  }



seeArtistDeezer(item: any) {
let artistId;
if(item.artist.type === 'artist')
{
artistId = item.artist.id;

}
else{
  artistId = item.artist[0].id;
 
}

this.router.navigate(['/deezer-artist', artistId]);

  }





  ngOnInit(): void {
  
  }




favPost(typefav: string, idfav: string, namefav: string, picture: string,namesong: string, nameapp: string, _uri: string)
{
this.typefav =   typefav;
this.idfav_ = idfav;
this.namefav_ = namefav;
this.picture_ = picture;
this.namesong_ = namesong;
this.nameapi = nameapp;
this.uri = _uri;


if(this.dato === null)
{
  alert("No puedes agregar a favoritos, inicia sesión para poder hacerlo");
}
else
{
  this.id = parseInt(this.dato);
  this.iduser_ = this.id;
  
  var data = {
    typeFav: this.typefav,
    idGetFav: this.idfav_ ,
    idUser: this.iduser_,
    songFav: this.namesong_,
    pictureFav: this.picture_,
    nameFav: this.namefav_ ,
    apiname: this.nameapi,
    uri: this.uri
}; 

this.serviceHome.favoritePost(data);
}


}

votePosts(typeVote_: string, idGetVote_: string, nameVote_:string, pictureVote_: string, api_: string)
{
  

  if(this.dato === null)
  {
    alert("No puedes votar, inicia sesión para poder hacerlo");
  }
  else
  {

this.latestpipe = this.datepipe.transform(this.date, 'yyyy-MM-ddTHH:mm:ss');  

    var data_ = {
      idUser: parseInt(this.dato),
      typeVote: typeVote_,
      idGetVote: idGetVote_,
      nameVote: nameVote_,
      pictureVote: pictureVote_,
      dateVote: this.latestpipe.toString(),
      apiname: api_
    };


this.serviceHome.votePost(data_);
  }
}



}
